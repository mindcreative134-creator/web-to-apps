...
class AssetEncryptor(private val secretKey: SecretKey) {
    ...
    fun encrypt(data: ByteArray, assetPath: String): ByteArray {
        ...
        return try {
            ...
            // 始终使用 Java AesCryptoEngine，确保与解密端 (AssetDecryptor.decryptWithKey) 格式完全一致
            // 注意：不要使用 NativeCryptoOptimized，因为：
            // 1. Native 引擎的输出格式可能与 Java AesCryptoEngine.decryptWithKey 不兼容
            // 2. 构建时宿主App可能有 libcrypto_engine.so 但导出的 Shell APK 可能被 strip 掉了
            // 3. 解密端 (AssetDecryptor) 只调用 AesCryptoEngine.decryptWithKey，不走 Native 路径
