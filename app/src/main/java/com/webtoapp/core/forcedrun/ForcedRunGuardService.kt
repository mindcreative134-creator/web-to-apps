...
class ForcedRunGuardService : Service() {
    ...
    companion object {
         ...
         * 启动守护服务
         * 
         * @param remainingMs 剩余运行时间（毫秒），用于限制 WakeLock 持有时长。
         *                    传 0 或负数表示未知时长（将使用 4 小时默认值）。
         */
        fun start(
            context: Context,
            packageName: String,
    ...
    /**
     * 获取 WakeLock，持有时长基于实际剩余时间 + 5 分钟缓冲
     * 
     * 避免固定 24 小时 WakeLock 导致的电量浪费和厂商杀进程策略触发。
     * 如果剩余时间未知（<=0），使用 4 小时默认值。
     */
    private fun acquireWakeLock() {
        try {
            ...
            val bufferMs = 5 * 60 * 1000L // 5 分钟缓冲
            val defaultDurationMs = 4 * 60 * 60 * 1000L // 默认 4 小时
            val maxDurationMs = 24 * 60 * 60 * 1000L // 绝对上限 24 小时
     ...
     * 动态更新 WakeLock 持有时长
     * 
     * 当外部更新了 remainingTimeMs 后可调用此方法刷新 WakeLock。
     */
    fun refreshWakeLock() {
        releaseWakeLock()
        acquireWakeLock()
