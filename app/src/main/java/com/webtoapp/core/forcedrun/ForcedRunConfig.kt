...
data class ForcedRunConfig(
    val enabled: Boolean = false,                    // 是否启用强制运行
    val mode: ForcedRunMode = ForcedRunMode.FIXED_TIME, // 运行模式
    
    // 固定时间段模式配置
    val startTime: String = "08:00",                 // 开始时间（HH:mm 格式）
    val endTime: String = "12:00",                   // 结束时间（HH:mm 格式）
    ...
    val showCountdown: Boolean = true,               // 显示剩余时间倒计时
    val allowEmergencyExit: Boolean = false,         // 允许紧急退出（连续点击5次特定区域）
    ...
    // 中断恢复
    val persistCountdown: Boolean = true             // 是否持久化倒计时进度（进程被杀后可恢复）
) {
    companion object {
        /** 禁用 */
